...
(function(){
	...
	com.github.ui = {
		...
		commitorLi: function(contributor) {
			...
			if (contributorType == 'anonymous') {
				...
				loginAndName = '<a href="http://github.com/' + contributor.login + '">' + contributor.login + '</a> '
		...
		showCommitor: function(contributor) {
			$("#result-list .list").append(self.commitorLi(contributor));
		},
		...
		showCommitsOnTimeline: function(commits, containerId) {
			...
			var placeholderId = $('<div/>', { 'id': 'visualization' });
	    	placeholderId.appendTo($(containerId));
	    	
	    	var overviewId = $('<div/>', { 'id': 'overview' });
			overviewId.appendTo($(containerId));
	
			var overviewLegendId = $('<div/>', { 'id': 'overviewLegend' });
			overviewLegendId.appendTo($(containerId));
	
			var options = {
				legend: { show: true, container: overviewLegendId },
		        bars: { show: true, fill: true, barWidth: 24 * 60 * 60 * 1000 },
				xaxis: { mode: "time", tickLength: 5, zoomRange: [0.1, 10] },
				selection: { mode: "x" },
		        zoom: { interactive: true },
	            grid: { markings: weekendAreas }
			};
	        var serie = { data: data, label: "last " + commitCount + " commits", color: "#aaa" };
	        ...
	        // setup overview
	        var overview = $.plot($(overviewId), [{ data: data, color: "#aaa" }], {
